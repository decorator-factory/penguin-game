...
/// Generated from file `test_level.tmj` on `2025-11-28 00:21:07.749986+00:00`.
use crate::levels;
use glam::vec2;
...
pub fn build(mut builder: levels::LevelBuilder) -> levels::Level {
    ...
    builder.rect_graphics("bricks_dark", vec2(120., 60.), vec2(72., 60.));
    builder.rect(Some("bricks"), vec2(108., 36.), vec2(96., 24.));
    builder.rect(Some("caution"), vec2(192., 60.), vec2(12., 60.));
    ...
    builder.polygon_trigger(levels::TriggerKind::Panic, &[vec2(-475., 724.), vec2(-468.667, 700.), vec2(-461., 699.667), vec2(-380.333, 749.667), vec2(-379., 762.), vec2(-416.667, 780.667), vec2(-453.667, 769.667), vec2(-475.667, 743.667)]);
    builder.polygon_trigger(levels::TriggerKind::Hello, &[vec2(552., -600.), vec2(588., -720.), vec2(672., -600.)]);
    builder.rect_trigger(levels::TriggerKind::ShowText("This one is tricky!\nBut still possible with one rocket jump.\nHarness the power of penguin jank."), vec2(1440., 96.), vec2(24., 24.));
    builder.rect_graphics("question_mark", vec2(-528., 756.), vec2(24., 24.));
    builder.rect_trigger(levels::TriggerKind::ShowText("Don't touch the banana!!"), vec2(-528., 756.), vec2(24., 24.));
    builder.level_start(vec2(-132., 96.));
