...
def trigger_action_expr(obj: dict[str, Any]) -> str:
    match extract_property(obj, "action", "string"):
        case "Panic":
            return "TriggerKind::Panic"
        case "Hello":
            return "TriggerKind::Hello"
        case "ShowText":
            text = extract_property(obj, "text", "string")
            return f"TriggerKind::ShowText(Rc::from({json.dumps(text)}))"
        case "SetEyepatch":
            enabled = extract_property(obj, "enabled", "bool")
            return f"TriggerKind::SetEyepatch({str(enabled).lower()})"
        case "Goto":
            ...
            return f"TriggerKind::Goto(LEVEL_OBJECT_{target_id}, StatusIcon::{status_icon})"
        case _:
